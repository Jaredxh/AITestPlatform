...
| 8000 | backend uvicorn | 是（开发 / 直接调 API 用） |
| 5432 | PostgreSQL | 默认暴露（生产可关，仅留容器网络） |
...
完整 API 文档：开发模式 `DEBUG=true` 下访问 `http://localhost:8000/docs`（生产模式自动关闭）。
...
curl -X POST -H "Authorization: Bearer <admin-token>" http://localhost:8000/api/admin/ui-media/cleanup
```
